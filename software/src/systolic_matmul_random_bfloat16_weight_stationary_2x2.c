...
int main(void) {
  ...
  static uint64_t a_tiles[WS_GEMM_A_TILE_WORDS_FOR(SA_ROWS, MAX_M, MAX_K)] __attribute__((aligned(64)));
  static uint64_t b_tiles[WS_GEMM_B_TILE_WORDS_FOR(SA_COLS, MAX_N, MAX_K)] __attribute__((aligned(64)));
  ...
  const ws_gemm_workspace_t workspace = ws_gemm_make_workspace_tiled(
      a_tiles, b_tiles, c_words, MAX_M, MAX_N, MAX_K, SA_ROWS, SA_COLS);
  ...
  srand(1);
  printf("=== Systolic GEMM Random Test (BF16 Weight-Stationary 2x2) ===\n");
