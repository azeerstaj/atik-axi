...
static float pattern_a(int i, int k) {
  return ((float)((k % 11) - 5) * 0.125f) + ((float)i - 0.5f) * 0.375f;
}

static float pattern_b(int k, int j) {
  return ((float)(((2 * k + j) % 13) - 6) * 0.15625f) - ((float)j - 0.5f) * 0.28125f;
}
...
static void print_float_matrix_2x2(const float C[SA_ROWS][SA_COLS]) {
  for (int i = 0; i < SA_ROWS; i++) {
    printf("[");
    for (int j = 0; j < SA_COLS; j++) {
...
int main(void) {
  ...
  static uint64_t a_tiles[WS_GEMM_A_TILE_WORDS_FOR(SA_ROWS, SA_ROWS, K)] __attribute__((aligned(64)));
  static uint64_t b_tiles[WS_GEMM_B_TILE_WORDS_FOR(SA_COLS, SA_COLS, K)] __attribute__((aligned(64)));
  ...
  const ws_gemm_workspace_t workspace = ws_gemm_make_workspace_tiled(
      a_tiles, b_tiles, c_words, SA_ROWS, SA_COLS, K, SA_ROWS, SA_COLS);
  ...
  printf("=== Systolic GEMM Simple Test (BF16 Weight-Stationary 2x2) ===\n");
  ...
  print_float_matrix_2x2(sw_ref_float);
  printf("HW C:\n");
  print_float_matrix_2x2(hw_float);
  printf("HW PERF:\n");
