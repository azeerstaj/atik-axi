...
#
# Copyright 2026
"""Export synthetic ViT inference cases as linkable C assets."""
...
DEFAULT_CASES = (
    VitCase("vit_l1_img32_p4_d64_h4_mlp128_cls10", 65, 48, 64, 4, 128, 1, 10, 901),
    VitCase("vit_l1_img64_p8_gray_d128_h4_mlp128_cls10", 65, 64, 128, 4, 128, 1, 10, 902),
    VitCase("vit_l1_grid11_p_synth_d128_h4_mlp256_cls10", 122, 128, 128, 4, 256, 1, 10, 903),
)
...
def make_case(case: VitCase) -> dict[str, list[int] | int]:
    if case.seq_len <= 1:
        raise ValueError(f"{case.name}: seq_len must include at least one patch plus CLS")
    if case.d_model % case.n_heads != 0:
        raise ValueError(f"{case.name}: d_model must be divisible by n_heads")
    if max(case.seq_len, case.patch_dim, case.d_model, case.hidden_dim, case.head_dim) > 256:
        raise ValueError(f"{case.name}: dimensions exceed current benchmark envelope")

    generator = torch.Generator(device="cpu")
    generator.manual_seed(case.seed)

    patches = uniform_scaled(generator, case.patch_count, case.patch_dim, scale=case.input_scale).to(torch.bfloat16)
    patch_w = uniform_scaled(generator, case.patch_dim, case.d_model, scale=case.weight_scale).to(torch.bfloat16)
    patch_b = uniform_scaled(generator, case.d_model, scale=case.bias_scale).to(torch.bfloat16)
    cls_token = uniform_scaled(generator, case.d_model, scale=case.input_scale).to(torch.bfloat16)
    position_embeddings = uniform_scaled(generator, case.seq_len, case.d_model, scale=case.input_scale).to(torch.bfloat16)
    ...
    patch_tokens = linear_bf16(patches, patch_w, patch_b)
    x = torch.cat([cls_token.reshape(1, case.d_model), patch_tokens], dim=0)
    x = (x.float() + position_embeddings.float()).to(torch.bfloat16)
    ...
    logits = linear_bf16(final_hidden[0:1], classifier_w, classifier_b).reshape(-1)

    return {
        "patches": bf16_bits(patches),
        "patch_w": bf16_bits(patch_w),
        "patch_b": bf16_bits(patch_b),
        "cls_token": bf16_bits(cls_token),
...
def print_case_sample(case: VitCase, data: dict[str, list[int] | int]) -> None:
    logits = data["expected_logits"]
    print(
        "SAMPLE,"
        f"{case.name},layers={case.n_layers},seq={case.seq_len},patch_dim={case.patch_dim},"
        f"d_model={case.d_model},heads={case.n_heads},head_dim={case.head_dim},"
        f"hidden={case.hidden_dim},classes={case.num_classes},"
        f"logit0={bf16_bit_to_float(logits[0]):.6f},"
        f"logit_last={bf16_bit_to_float(logits[-1]):.6f}"
    )


def write_assets(cases: tuple[VitCase, ...], out_dir: Path) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    header = out_dir / "vit_cases.h"
    source = out_dir / "vit_cases.c"
    generated = [(case, make_case(case)) for case in cases]

    max_seq_len = max(case.seq_len for case in cases)
    max_patch_count = max(case.patch_count for case in cases)
    max_patch_dim = max(case.patch_dim for case in cases)
    ...
    print("SAMPLE_HEADER,name,layers,seq,patch_dim,d_model,heads,head_dim,hidden,classes,logit0,logit_last")
    for case, data in generated:
        print_case_sample(case, data)

    with header.open("w", encoding="utf-8") as out:
        out.write("#ifndef GIRDAP_GENERATED_VIT_CASES_H\n")
        out.write("#define GIRDAP_GENERATED_VIT_CASES_H\n\n")
        out.write("#include <stdint.h>\n\n")
        out.write(f"#define VIT_MAX_SEQ_LEN {max_seq_len}\n")
        out.write(f"#define VIT_MAX_PATCH_COUNT {max_patch_count}\n")
        out.write(f"#define VIT_MAX_PATCH_DIM {max_patch_dim}\n")
        out.write(f"#define VIT_MAX_D_MODEL {max_d_model}\n")
        out.write(f"#define VIT_MAX_HEADS {max_heads}\n")
        out.write(f"#define VIT_MAX_HEAD_DIM {max_head_dim}\n")
        out.write(f"#define VIT_MAX_HIDDEN_DIM {max_hidden_dim}\n")
        out.write(f"#define VIT_MAX_LAYERS {max_layers}\n")
        out.write(f"#define VIT_MAX_NUM_CLASSES {max_num_classes}\n\n")
        out.write("typedef struct {\n")
        out.write("  const char *name;\n")
        out.write("  int seq_len;\n")
        out.write("  int patch_count;\n")
        out.write("  int patch_dim;\n")
        ...
        fields = (
            "patches", "patch_w", "patch_b", "cls_token", "position_embeddings",
            "ln1_gamma", "ln1_beta", "ln2_gamma", "ln2_beta", "wq", "wk", "wv",
            "wo", "w1", "w2", "bq", "bk", "bv", "bo", "b1", "b2",
            "final_ln_gamma", "final_ln_beta", "classifier_w", "classifier_b",
            "expected_logits",
        )
        for field in fields:
            out.write(f"  const uint16_t *{field};\n")
        out.write("} vit_case_t;\n\n")
        out.write("extern const vit_case_t vit_cases[];\n")
        out.write("extern const int vit_case_count;\n\n")
        out.write("#endif\n")
    ...
    with source.open("w", encoding="utf-8") as out:
        out.write('#include "vit_cases.h"\n\n')
        for idx, (case, data) in enumerate(generated):
            prefix = f"vit_case_{idx}"
        ...
        out.write("const vit_case_t vit_cases[] = {\n")
        for idx, (case, data) in enumerate(generated):
            prefix = f"vit_case_{idx}"
            out.write("  {\n")
            out.write(f'    .name = "{case.name}",\n')
            out.write(f"    .seq_len = {case.seq_len},\n")
            out.write(f"    .patch_count = {case.patch_count},\n")
            out.write(f"    .patch_dim = {case.patch_dim},\n")
            ...
            out.write("  },\n")
        out.write("};\n\n")
        out.write("const int vit_case_count = sizeof(vit_cases) / sizeof(vit_cases[0]);\n")
...
def main() -> None:
    ...
    print(f"wrote ViT cases to {args.out_dir} using PyTorch backend")
