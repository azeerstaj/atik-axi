...
class SystolicArrayFpgaSafe8x8MemImpl(
  outer: SystolicArrayFpgaSafe8x8MemRoCC
  ...
  val aBuf = SyncReadMem(maxK, UInt(cacheDataBits.W)) // a vector
  val bBuf = SyncReadMem(maxK, UInt(cacheDataBits.W)) // b vector
  ...
  val (s_idle :: s_req_fill_b :: s_wait_fill_b :: s_req_fill_a :: s_wait_fill_a ::
    s_compute_read :: s_compute :: s_softmax_load :: s_softmax_max :: s_softmax_sum :: s_softmax_norm ::
    s_quantize :: s_req_put :: s_wait_put :: s_resp :: Nil) = Enum(15)
  ...
  // read from buffers
  val aReadData = aBuf.read(computeIdx(bufIdxWidth - 1, 0), state === s_compute_read)
  val bReadData = bBuf.read(computeIdx(bufIdxWidth - 1, 0), state === s_compute_read)
  val aLanes = Wire(Vec(nRows, UInt(precision.W)))
  val bLanes = Wire(Vec(nCols, UInt(precision.W)))
  for (r <- 0 until nRows) {
    aLanes(r) := aReadData(precision * (r + 1) - 1, precision * r)
  }
  for (c <- 0 until nCols) {
    bLanes(c) := bReadData(precision * (c + 1) - 1, precision * c)
  }
  ...
  when(state === s_wait_fill_b) {
    ...
    when(tlOut.d.fire) {
      ...
      bBuf.write(fillIdx(bufIdxWidth - 1, 0), tlOut.d.bits.data(cacheDataBits - 1, 0))
  ...
  when(state === s_wait_fill_a) {
    ...
    when(tlOut.d.fire) {
      ...
      aBuf.write(fillIdx(bufIdxWidth - 1, 0), tlOut.d.bits.data(cacheDataBits - 1, 0))
      val nextIdx = fillIdx + 1.U
      when(nextIdx >= totalK) {
        fillIdx := 0.U
        computeIdx := 0.U
        incChunkStarted := true.B
        state := s_compute_read
      }.otherwise {
        fillIdx := nextIdx
        state := s_req_fill_a
      }
...
class SystolicArrayFpgaSafe8x8MemRoCC(
  opcodes: OpcodeSet,
  ...
  val clientName: String = "SystolicArrayFpgaSafe8x8MemRoCC"
)(implicit p: Parameters) extends LazyRoCC(opcodes) {
  override lazy val module = new SystolicArrayFpgaSafe8x8MemImpl(this)
